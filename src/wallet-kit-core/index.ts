...
import { getWallets, isWalletWithSuiFeatures } from '../wallet-standard/index.js';
...
const SUI_WALLET_NAME = 'BenFen Wallet';
...
export function createWalletKitCore({
	...
	const walletKit: WalletKitCore = {
		...
		signMessage(messageInput) {
			...
			if (!internalState.currentWallet.features['bfc:signMessage']) {
				throw new Error('Wallet does not support deprecated `signMessage` method.');
			}

			return internalState.currentWallet.features['bfc:signMessage'].signMessage({
				...messageInput,
		...
		signPersonalMessage(messageInput) {
			...
			if (!internalState.currentWallet.features['bfc:signPersonalMessage']) {
				throw new Error('Wallet does not support the new `signPersonalMessage` method.');
			}

			return internalState.currentWallet.features['bfc:signPersonalMessage'].signPersonalMessage({
				...messageInput,
				account: messageInput.account ?? internalState.currentAccount,
			});
		},
		...
		async signTransactionBlock(transactionInput) {
			...
			return internalState.currentWallet.features['bfc:signTransactionBlock'].signTransactionBlock({
				...transactionInput,
				account,
				chain,
			});
